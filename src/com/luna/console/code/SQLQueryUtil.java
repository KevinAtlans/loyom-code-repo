...
public class SQLQueryUtil  {
     ...
     private static String USE_JDBC_DRIVER = "com.mysql.jdbc.Driver"; 
    /**
     * main()
     *
     * @param args
    ...
    public String toSQL() {
        ...
        if (columnMap.isEmpty()) {
            ...
            for (String key : columnMap.keySet()) {
                this.append(key);
                if (StrUtil.isNotEmpty(columnMap.get(key))) {
        ...
        for (String key : tableMap.keySet()) {
            this.append(key);
            if (StrUtil.isNotEmpty(tableMap.get(key))) {
        ...
        if (isDebug) {
            Print.print(sqlResult.toString().replace(" ,", ",").replace("  ", " ").replace(" 1 = 1 AND", "").replace(" 1 = 1 OR", ""));
        }
