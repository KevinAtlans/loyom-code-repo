...
/**
 * LoginThread handles: - Listening for login activity - Validating logins -
 * Creating "PlayerChannels" from login information - Register player channels
 * with the command thread's selector - Putting player channels in the list of
...
class LoginThread extends Thread {
    ...
    private final Selector commandSelector;

    private final ConnectionList<SocketChannel> acceptedConnections;

    // private ConnectionList<SocketChannel> authConnections;
    private final LinkedList<PlayerChannel> playerChannelList;

    // private LinkedList <Player>playerList;
    private final Selector loginSelector;
    ...
    /*
     * We pass in: - the selector for login connections - the selector to pass
     * channels to when they've logged in - a list of accepted connections where
     * the acceptor thread dumps it's recent connections - a list of player
     * channels that the Command Listener watches for logged in channels - a
     * list of players that the game engine watches for
     */
    public LoginThread(Selector loginSelector, Selector commandSelector,
        ...
        try {
            ...
            System.out
                    .println("LoginThread() --> No players loaded, could not connect to database.");
        ...
        /*
         * try{ mSqlConn.getPlayers(playerList); mSqlConn.close();
         * }catch(SQLException se){ System.out.println("LoginThread() --> " +
         * se); }finally{ mSqlConn = null; }
         */
        // Scratch that: we'll grab the players from the database (along with
        // slots, items, etc.) when they log in

        // DEBUG: go through the list now that it's filled and print out the
        // names
        // to be sure it actually GOT filled
        // for(Iterator i = playerList.iterator(); i.hasNext();){
        // System.out.println(((Player)i.next()).getPassword() +
        // "has been verified.");
        // }
    }

    /**
     * Listen for any connections, prompt players for usernames and passwords,
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                // 5) check for player in [player collection] with matching
                // login/password
                // NOTE: we don't want to check the login before they've sent
                // the password,
                // because we don't want them to know if they even got the login
                // right

                // DEBUG:
                // System.out.println("LoginThread: Selecting");
                // System.out.println("Never ending loop iteration #" +
                // ++iIterations);

                // register any new SocketChannels encountered by the Acceptor
                // thread
                ...
                // block and wait for input from any of the registered socket
                // channels
                ...
                if (keysReady > 0) {
                    ...
                    // DEBUG:
                    // System.out.println(keysReady +
                    // " Keys ready in never ending loop iteration #" +
                    // iIterations + "\n\n\n");
                    ...
                    if (loginSelector.selectedKeys().size() > 0) {
                        // DEBUG:
                        System.out
                                .println("Removing leftover ready keys with: loginSelector.selectedKeys().clear()");
    ...
    /**
     * rip through the list of connections and register them all with the
     * commandSelector, which I THINK tells us that there is data (e.g. a
     * command, in this program) waiting for us
     * 
     * @throws Exception
     *             no idea what exception this throws
     */
    protected void registerNewChannels() throws Exception {

        SocketChannel channel;

        // work through all the socket channels put into the acceptedCOnnections
        // linked list
        ...
        while (null != (channel = acceptedConnections.removeFirst())) {
            ...
            // at this point, I think we lose the player object (gets replaced
            // with a login?)
            // nope, at this point there is not object attached to the socket
            // channels key

            // register the socket channel with the login selector and attach a
            // new login
            ...
            // - get the key from the selector we just registered this channel
            // with
    ...
    /**
     * for each channel that has received a command, rip through and process the
     * command
     * 
     * @throws Exception
     *             no idea what this throws
     */
    protected void acceptPendingRequests() throws Exception {
        ...
        for (Object readyKey : readyKeys) {
            ...
            // process the incoming data from the socket channel associated with
            // that key
            handleInput(key);

            // DEUBG: not getting here for some reason? (Dec 7, 2002)
            // System.out.println("Done Handling Input: " +
            // ((Login)key.attachment()).getCurrentStateString());
    ...
    /**
     * Process the incoming data from a socket channel whose selector key is in
     * the "ready" state
     * 
     * @param key
     *            Key associated with the socket channel that has recieved data
     * @throws Exception
     *             no idea what exception this throws
     */
    protected void handleInput(SelectionKey key) throws Exception {
        ...
        try {
            ...
            // if we've gotten a key, with no attached bytes, then the user's
            ...
            // set the limit to the last byte read and reset the position to the
            // beginning
            readBuffer.flip();
            ...
            if (strRequest.endsWith("\r\n")) {
                ...
                // Since the latest login string ended in [enter], pass the
                // whole string buffer
            ...
            if (!incomingChannel.isConnected()) {
                ...
                // DEBUG:
                System.out
                        .println("LoginThread: Cancelling the socket key registration for socket: "
                                + incomingChannel.socket().getInetAddress().toString());

                // There was an error so cancel the "ready" key and disconnect
                // the socket channel
                // from our end
    ...
    /**
     * Store the latest login information. If the latest login information is a
     * password then attempt to validate the user.
     * 
     * @param loginString
     *            Complete login information (either a login or a password)
     * @param channel
     *            Socket Channel that the user is on
     * @throws java.io.IOException
     *             if there io problems
     */
    protected void handleCompletedLoginState(String loginString, SocketChannel channel)
            throws IOException {

        Player player;

        // get the login object from the "ready" login selectors "ready" key
        // associated with this Socket Channel
        Login login = (Login) channel.keyFor(loginSelector).attachment();

        // DEBUG:
        // System.out.println("Login is: (" + login.getLogin() + ", " +
        // login.getPassword() + ")");
        ...
        if (tok.hasMoreTokens()) {
            ...
            // DEBUG:
            // System.out.println("LoginThread: handleCompletedLoginState: login/pwd = "
            // + loginString);

            // DEBUG:
            // System.out.println("LoginThread: handleCompletedLoginState: state is "
            // + login.getState() + " (" + login.PASSWORD + ")");

            // If the player has entered their password then attempt to validate
            // the user
            if (login.getState() == Login.PASSWORD) {
                ...
                if (null != (player = (checkLogin(login)))) {
                    ...
                    try {
                        // the player is logged in so send them a welcome
                        // message
                        channel.write(getWelcomeMessage(player.getName()));
                    } catch (Exception e) {
                        System.out
                                .println("LoginThread: handleCompletedLoginState: getWelcomeMessage() failed");
                    }
                    ...
                    // create playerChannel and push onto playerChannel linked
                    // list
                ...
                // if they haven't exhausted their login attempts, print the
                // login again.
                try {
                    channel.write(getLoginRequest());
                } catch (Exception e) {
                    System.out
                            .println("LoginThread: handleCompletedRequest: channel.write(login request) error");
                    e.printStackTrace();
                }

                // else the login state is LOGIN, so move on to the password
                // prompt
            } else {
                // DEBUG:
                ...
                try {
                    ...
                    System.out
                            .println("LoginThread: handleCompletedRequest: channel.write(pwd request) error");
                    e.printStackTrace();
                }
            }
        }
    ...
    /**
     * Check a login object's username and password against all the users in the
     * database to see if we get a match
     * 
     * @param login
     *            Login object containing username and password to use to match
     *            against all users
    ...
    private Player checkLogin(Login login) {
        ...
        // DEBUG:
        // System.out.println("Checking login (" + login.getLogin() + ", " +
        // login.getPassword() + ")");
        ...
        try {
            player = mSqlConn
                    .getPlayer(login.getLogin().toString(), login.getPassword().toString());
            mSqlConn.close();
    ...
    /*
     * // KEEP THIS: It's example of how to construct a buffer protected
     * ByteBuffer sendError(StringBuffer strBuf) throws Exception {
     * 
     * CharBuffer chars = CharBuffer.allocate(100); chars.put("You sent: [" +
     * strBuf.toString() + "]\r\n"); chars.put("Here's the response: \r\n");
     * chars.put("(none yet)\r\n"); chars.flip();
     * 
     * // Translate the Unicode characters into ASCII bytes. ByteBuffer buffer =
     * ascii.newEncoder().encode(chars); buffer.rewind(); return buffer; }
     */
}
