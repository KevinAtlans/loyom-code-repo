...
import com.luna.console.jmud.Flag;
...
/*
 * Class: Set Purpose: Sets a player property
 */
public class Set extends Command {
    ...
    public boolean exec() {
        ...
        // System.out.println("Set command called for plaery " +
        // player.getName() + " with target " + target);
        ...
        if (target.length() == 0) {
            ...
            for (Field f : Player.getFlags()) {
                ...
                try {
                    ...
                    strbMessage.append(f.getAnnotation(Flag.class).name()).append(":\t")
                            .append(f.getBoolean(player) ? "on" : "off").append(CRLF);
                    ...
                    strbMessage.append("[error retrieving ").append(f.getName()).append("]")
                            .append("\n");
                    System.out.println("Couldn't retrieve setting for flag [" + f.getName()
            ...
            // if they've only supplied the flag name then simply print the name
            // and value
            if (args.length == 1) {

                // display the annotated name and value ("on" or "off") of the
                // field
                try {
                    strbMessage.append(field.getAnnotation(Flag.class).name()).append(":\t")
                            .append(field.getBoolean(player) ? "on" : "off").append(CRLF);
                } catch (Exception e) {
                    strbMessage.append("[error retrieving ").append(args[0]).append("]");
                    System.out.println("Couldn't retrieve setting for flag [" + field.getName()
                            + "] for player [" + player.getName() + "]");
                }
                ...
                try {
                    ...
                    strbMessage.append(field.getAnnotation(Flag.class).name()).append(" is now:\t")
                            .append(field.getBoolean(player) ? "on" : "off").append(CRLF);
