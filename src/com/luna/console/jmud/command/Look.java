...
/*
 * Class: Look Purpose: Extends the Command class to provide the functionality
 * for a player to look at something
 */
public class Look extends Command {
     ...
     * 
     * @param pc
     *            The PlayerChannel to send the results too
     * @param r
     *            The current room
     * @param target
     *            target to look at, if null or empty string, looks at current
     *            room
     */
     ...
     * <p/>
     * If the target is null or blank look at the room, else if the target can
     * be mapped to an item, player or mob, look at the item, player or mob
     */
    @Override
    public boolean exec() {
        ...
        if (strTarget == null || strTarget.length() == 0) {
            message.append(room.getShortDescription())
                    ...
                    .append(CRLF)
                    .append(mobAndPlayerNames.length() != 0 ? ALSO_HERE_PROMPT + mobAndPlayerNames
                            + CRLF : "")
        ...
        /*
         * for each of the hashtables holding players, mobs and items, if the
         * hashtable holds a key (always a String: the name) that starts with
         * strTarget, get the description of the object whose key started with
         * strTarget
         */
        ...
        if (matches == 0) {
            message.append("You don't see a ").append(strTarget).append(".");
            sendMessage(message);
            return true;
        }

        // if we found more than one match then prompt the player for which
        // thing they meant
        if (matches > 1) {
    ...
    private void sendMessage(StringBuilder strbMessage) {
        ...
        // tack on the exits and the prompt even though the engine is supposed
        // to handle the
        // prompt.
        strbMessage.append(room.getExits()).append(CRLF).append(player.getPrompt());
