...
public class Get extends Command {
    ...
    public boolean exec() {
        ...
        if (strTarget == null || strTarget.length() == 0) {
            strbMessage.append("You must specify an item to get").append(CRLF);
            // check if the player has a free hand
        } else if (!player.hasFreeGrabber()) {
            strbMessage.append("You need a free hand to grab that.").append(CRLF);
            // make sure the item is in the room
        } else if (room.getItem(strTarget) == null
                || (item = room.remove(room.getItem(strTarget))) == null) {
            strbMessage.append("You don't see a ").append(strTarget).append(CRLF);
            // nothing failed, we got the item
        } else {
            player.grab(item);
            strbMessage.append("You get the ").append(item.getName()).append(CRLF);
        }

        // tack on the player's prompt even though the engine is supposed to
        // handle the
        // prompt.
