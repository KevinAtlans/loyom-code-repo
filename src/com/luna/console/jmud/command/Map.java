...
import com.luna.console.jmud.MysqlConnector;
...
public class Map {
    ...
    // rooms connected to already-being-drawn rooms to check for further
    // connected rooms
    ...
    public void loadRooms(int iPlayerID) {
        ...
        try {
            ...
            // fill the room array and store what rooms are where in the hash
            // table
    ...
    /**
     * Fill a 2D char array with characters representing rooms and connections
     * between rooms
     */
    public void fillMap() {
        ...
        /*
         * for the number of rooms, we may just use this number (iRooms) to
         * determine how many rooms from memory, not from line of sight
         */
        Room currRoom;
        Room tempRoom;
        ListIterator iter;
        ...
        // until I figure out why I used the last room I'll just go with the
        // first room
        ...
        // (we have to do it in the loop down below anyways, so we'll not do it
        // here)
        // listAddedRoomIDs.add(new
        // Integer(((Room)listRooms.getFirst()).getID()));

        // add all the rooms that we grabbed from the database to the
        // "rooms to be drawn" list and the
        // "rooms to be processed" list
        // except the first one which is already the room currently being
        // processed
        for (int i = 1; i < rooms.length; i++) {
            // DEBUG:
            System.out
                    .println("adding room [" + rooms[i].getID() + "] to the to-be-processed list");
        ...
        while (currRoom != null) {
            ...
            // null the current room so we can test to see if we get the next
            // one
            currRoom = null;
            ...
            // keep getting the next room until we find one that hasn't been
            // processed
            while (!listRoomsToProcess.isEmpty() && currRoom == null) {
                ...
                // get the room's ID to make sure we haven't ALREADY processed
                // it (not that it would hurt)
                while (iter.hasNext()) {
                    // check the next processed ID in the list against the
                    // current room ID
                    if (((Integer) iter.next()).intValue() == currRoom.getID()) {

                        // we've found the ID has already been processed
                        // so set the current room to null so the outer loop
                        // will grab the next room
                        currRoom = null;

                        // we found the room had been processed so we don't need
                        // to keep looping
        ...
        // in order to hold 1 room and 1 possible connection in any direction
        // we'll
        ...
        // one full row and one full column to account for the connections that
        // aren't shared
        /*
         * Example (Zeros are rooms, dashes/pipes are connections between them)
         * 
         * | 2 rooms high, but 2 * 2 + 1 rows high -0-0- | 0-0- |
         * 
         * ++ Columns for first column of rooms ++ Columns for second column of
         * rooms ++ Columns for third column of rooms + Column for left over,
         * "unshared" connection column
         * 
        ...
        // (I think when we did it before we got an error when we change the
        // list afterwards)
        // (Yup, works now)
        ...
        while (iter.hasNext()) {
            ...
            // correct edge rooms that get messed up by the above formula (i.e.
            // where actual room X = min X, work it out to see what I mean)
            ...
            // NOTE: This will overwrite previous connections if they exist in
            // the
    ...
    public String toString() {
        ...
        // to the beginning of the row array, but it has to do with the
        // difference
        // between how my map grid looks and how the rooms will be added to the
        // array)
        // I have to draw the TOP row of the map FIRST (i.e. the GREATEST Y
        // coord first)
        // in order to have the map printed right-side up.
        for (int iRows = iMapHeight; iRows > 0; iRows--) {
            for (int iCols = 0; iCols < iMapWidth; iCols++) {
                strbMap.append(cMap[iCols][iRows - 1]); // X,Y coords, X is the
                                                        // column, Y is the row
            }
    ...
    private boolean processRoom(Room room) {
        ...
        // run through the list of processed room IDs and check if this one is
        // there
        ...
        // we've not processed this room yet so we'll add it to the following
        // lists
        // - rooms to draw
        ...
        // DEBUG:
        System.out.println("Added room " + room.getID() + " at " + room.getCoords()[X] + ","
                + room.getCoords()[Y]);
