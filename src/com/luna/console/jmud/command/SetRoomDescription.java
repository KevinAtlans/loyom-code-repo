...
/**
 * Executable command "Set Room Description" for setting the description of a
 * room in memory and in the database.
 * 
...
public class SetRoomDescription extends Command {
    ...
    public boolean exec() {
        ...
        try {
            playerChannel.sendMessage("This command does not work yet, sorry. " + CRLF
                    + room.getExits() + CRLF + player.getPrompt());
        } catch (Exception e) {
            System.out.println("Couldn't print \"command no workee\" message to "
                    + player.getName());
        }
