...
public class PlayerChannel {
    ...
    // message
    // to a player)
    private final ChannelWriter cw = new ChannelWriter();
    ...
    /**
     * Compare this PlayerChannel with another PlayerChannel to see if they are
     * equal
     * 
     * @param obj
     *            Object to compare with this PlayerChannel (equals must be
     *            usable from any object)
    ...
    public boolean equals(Object obj) {
        ...
        if (obj.getClass() == this.getClass() && ((PlayerChannel) obj).player.equals(this.player)) {
            return true;
        }
        return false;
    }
    ...
    /*
     * This seems a little dangerous because we can't really *guarantee* that
     * the channel will be unique, but we can guarantee that the player is
     * unique based on the player's hashcode. The only time we'll hit a problem
     * with this (I think) is when one player has multiple channels, ... but
