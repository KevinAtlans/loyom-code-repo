...
class CommandListenerThread extends Thread {
    ...
    private final Hashtable<String, Constructor> commandAliases = new Hashtable<String, Constructor>();

    /**
     * - Creates a new map - Loads the map with rooms, players, monsters, etc. -
     * Loads command aliases (e.g. e, ea, eas and east for "East")
     * 
     * @param readSelector
     *            Watches for new data on attached sockets (SocketChannels)
     * @param playerChannels
     *            A list of attached sockets for authenticated players
     * @param commands
     *            The list where we'll put command objects created from incoming
     *            data
     * @throws Exception
     *             Exception, ClassNotFoundException
     */
    public CommandListenerThread(Selector readSelector, LinkedList<PlayerChannel> playerChannels,
        ...
        try {
            ...
            System.out
                    .println("getCommandAliases() --> Could not connect to the database. returning."
                            + se);
            return;
        }
        ...
        // ea
        // eas
        // east - we treat the full command as an alias (this is basically
        // semantics)
        try {
            ...
            // loop through each command alias (2-element array containing the
            // alias-command pair)
            while (i.hasNext()) {
                // DEBUG: this prints out a line of dots as command aliases get
                // processed
                System.out.print(".");
                ...
                // get the command constructor that matches this alias using the
                // full command name
                ...
                // constructor object to the command alias commandAliases and
                // key it on the alias
                if (con != null) {
                    ...
                    // " alias added with constructor " + con.getName());
                }
            }
    ...
    /**
     * Listen for input on active connections using a Selector. When the
     * Selector indicates there is activity on a SocketChannel, process the
     * input.
     */
    @Override
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                // wait for active keys (.select() blocks) and process their
                // input
                if (readSelector.select() > 0) {
                    handleReadyKeys();
                    readSelector.selectedKeys().clear();

                    // why would we Selector.select() return if their were no
                    // active keys?
                } else {
    ...
    /**
     * The login thread puts incoming SocketChannels into the "playerChannel"
     * linked list once the players authenticate themselves.
     * <p/>
     * This function goes through each SocketChannel and registers it with the
     * "read" selector which will then list for incoming data from that
     * authenticated players socket channel
     * 
     * @throws Exception
     *             if the networking channel code come off the rails
     */
    protected void registerNewChannels() throws Exception {
        ...
        for (Iterator i = playerChannelList.iterator(); i.hasNext();) {
            ...
            // the user has only logged on so far, so add them (i.e. their
            // player channel)
            ...
            // show the player the description of their current room and their
            // prompt
            try {
                ...
                System.out
                        .println("CommandListenerThread: parseCommand: failed channelwriter.sendMessage()");
            }
            i.remove();
        }
    }
    ...
    /**
     * Each socket channel that sends input has it's "key" marked Active.
     * <p/>
    ...
    /**
     * Grab the data from a socket channel that has it's "key" set to "ready".
     * Process the data to see if it adds to a command or completes a command.
     * Append non-completeing data to a stored command and create command
     * objects for completed commands. Store completed commands in the command
     * execution queue.
     */
    protected void parseCommand(SelectionKey key) throws Exception {
        ...
        try {
            // if the socket channel activates it's key but doesn't send any
            // data
            ...
            if (strRequest.endsWith("\n") || strRequest.endsWith("\r\n")) {
                ...
                // if the latest request string ended in [enter], pass the whole
                // string buffer
                parseCompletedCommand(requestString.toString(), incomingChannel);

                /*
                 * try{
                 * //System.out.println(((Player)incomingChannel.keyFor(readSelector
                 * ).attachment()).getPrompt());
                 * channelwriter.sendMessage(strResult + "\n\r",
                 * incomingChannel);
                 * channelwriter.sendMessage(((Player)incomingChannel
                 * .keyFor(readSelector).attachment()).getPrompt(),
                 * incomingChannel); }catch(Exception e){ System.out.println(
                 * "CommandListenerThread: parseCommand: failed channelwriter.sendMessage()"
    ...
    /**
     * Once a command has been fully entered, or completed, meaning that the
     * player sent a CR or CR+LF, handle the full command.
     * <p/>
     * Handling a command consists of retrieving the command's constructor from
     * a hashtable containing all commands mapped to all their aliases, creating
     * an instance of the command and adding it to the command queue of the game
     * engine.
     * 
     * @param request
     *            The command to parse
     * @param channel
     *            The SocketChannel belonging to the player that sent the
     *            command
     */
    protected void parseCompletedCommand(String request, SocketChannel channel) throws IOException {
        ...
        /*
         * The users data has to start with a command alias but after that there
         * can be "target" data (e.g. "look Bob"). We'll tokenize the string so
         * that we can easily pick of the first whole word
         */
        if (tok.countTokens() >= 1) { // if they've sent a valid command ...
            ...
            // DEBUG:
            // System.out.println("getting constructor for command: " +
            // strCommand);

            /*
             * since we loaded all the constructors in the commandAliases we can
             * just pull out the constructor that matches this alias
             */
            ...
            if (constr != null) {
                try {
                    // create a new instance of the specified command class
                    // using the
                    // player, the players room and the target string (e.g.
                    // "Bob" from "look Bob")
                    cmd = (Command) constr.newInstance(pc, r, strbTarget.toString());
                    commands.add(cmd);

                    // catch all the different types of errors for debugging
                    // purposes:
                    ...
                    // DEBUG:
                    System.out
                            .println("Invocation Target Exception\nCouldn't create instance of Class \""
                                    + constr.getName() + "\"");
     ...
     * 
     * @param className
     *            Name of the class (should be a command class) to create the
     *            constructor for
     */
    private Constructor getConstructor(String className) throws ClassNotFoundException {
        ...
        try {
            ...
            System.out.println("Exception getting class for " + className + ": " + e.getMessage()
                    + "\n" + e);
            return null;
        }
        ...
        if (c != null) {
            try {
                /*
                 * get the constructor for the class that matches the method
                 * sig: className(Player, Room, String)
                 */
                return c.getConstructor(new Class[] {// Class.forName("PlayerChannel"),
                PlayerChannel.class,
                        // Class.forName("Room"),
                        Room.class, String.class });
                // check for various exceptions to aid in debugging
                /*
                 * We're not using "Class.forName(<classname>)" anymore
                 * }catch(ClassNotFoundException cnfe){ // DEBUG:
                 * System.out.println("Couldn't find all Classes for Class \"" +
                 * c.getName() + "\""); return null;
                 */
