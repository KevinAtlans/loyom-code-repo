...
public class Room {
    ...
    private final Hashtable<Object, PlayerChannel> playerChannels = new Hashtable<Object, PlayerChannel>(
            PLAYERS);

    private final Hashtable<String, PlayerChannel> playerChannelsByName = new Hashtable<String, PlayerChannel>(
            PLAYERS);

    private final Hashtable<Mob, Mob> mobs = new Hashtable<Mob, Mob>(MOBS);

    private final Hashtable<String, List<Mob>> mobsByAcronym = new Hashtable<String, List<Mob>>(
            MOBS);

    private final Hashtable<String, List<Mob>> mobsByName = new Hashtable<String, List<Mob>>(MOBS);

    // key the items on Object so that we can use the item itself, or the item's
    // name, for the key
    private final Hashtable<Item, Item> items = new Hashtable<Item, Item>(ITEMS);

    private final Hashtable<String, List<Item>> itemsByName = new Hashtable<String, List<Item>>(
            ITEMS);

    // used for writing to a players SocketChannel
    // (which is wierd because why wouldn't the PlayerChannel be used to send a
    // message
    // to a player)
     ...
     * 
     * @param strShortDesc
     *            The short description of the <CODE>Room</CODE>
     * @param strLongDesc
     *            The long description of the <CODE>Room</CODE>
     */
    ...
    /**
     * Creates a <CODE>Room</CODE> with short and long descriptions, an Id and
     * x, y and z coordinates.
     * 
     * @param strShortDesc
     *            The short description of the <I>room</I>
     * @param strLongDesc
     *            The long description of the <I>room</I>
     * @param iID
     *            The database ID of the <I>room</I>
     * @param iX
     *            The x coordinate of the <I>room</I>
     * @param iY
     *            The y coordinate of the <I>room</I>
     * @param iZ
     *            The z coordinate of the <I>room</I>
     */
     ...
     * 
     * @param room
     *            The <CODE>Room</CODE> object to add
     * @param dir
     *            The acronym of the direction to add the <CODE>Room</CODE>
     */
    public void addRoom(Room room, String dir) {
        ...
        try {
            switch (dir.charAt(0)) {
            case 'N': // n
                // make sure "N" doesn't come first, because N,NE,NW all START
                // with "N"
     ...
     * 
     * @return A string that lists the exit accronyms (e.g. e, w, etc.) for this
     *         room
     */
    ...
    /**
     * Get the short description for this room. Each room has a short
     * description and a long description so that users can choose the amount of
     * detail that they want.
     * 
    ...
    /**
     * Set the short description for this room. Each room has a short
     * description and a long description so that users can choose the amount of
     * detail that they want.
     * 
     * @param strShortDescription
     *            The short description for this room
     */
     ...
     * and a long description so that users can choose the amount of detail that
     * they want.
     * 
     * @return The short description for this room
     */
    ...
    /**
     * Get the visibility "index" for this room. Visibility refers to how this
     * room affects players' abilities to see through it. This comes into play
     * when a player wants to see or map adjacent rooms without having been in
     * them (in which case they would be in their memory - which is stored in
     * the database)
     * 
     * @return The short description for this room
     */
    ...
    /**
     * Set the visibility "index" for this room. Visibility refers to how this
     * room affects players' abilities to see through it. This comes into play
     * when a player wants to see or map adjacent rooms without having been in
     * them (in which case they would be in their memory - which is stored in
     * the database)
    ...
    /**
     * Checks the hashtable for a PlayerChannel with a matching player
     * 
     * @param p
     *            player to look for in this room
     * @return true if the player is found, false if the player is not found
     */
    public boolean hasPlayer(Player p) {

        /*
         * Now this is risky because we're assuming that the hashcode of a
         * player channel will always be the same as the hashcode for a player.
     ...
     * 
     * @param partialName
     *            The start of the player's partialName to look for in this room
     * @return The <code>Player</code>'s partialName if a <code>Player</code> is
     *         found else null.
     */
    public List<PlayerChannel> getPlayers(String partialName) {
        ...
        // check the start of each String (i.e. not Player hashcodes) key
        // against partialName
        for (String fullName : playerChannelsByName.keySet()) {
            // Check if any the current player partialName starts with the
            // partialName provided
            if ((fullName).toLowerCase().startsWith(partialName)) {
                // since we found a match, add that partialName to the set of
     ...
     * 
     * @return <code>true</code> if the <code>Mob</code> is in the room, or
    ...
    /**
     * Checks the <code>Mob</code> hashtable for matching <code>Mob</code>
     * names. If there is a matching acronym, we'll just return the one
     * partialName. If there are more than one full names that match (i.e. they
     * just typed the beginning of the partialName) then we'll return a hashset
     * of them all (no dups in a set).
     * 
     * @param partialName
     *            The partialName or acronym of the <code>MobType</code> of the
     *            <code>Mob</code> to match
     * @return a hashset containing either one Mob partialName that matched the
     *         acronym or all the <code>Mob</code> names whose beginnings match
    ...
    public List<Mob> getMobs(String partialName) {
        ...
        // check the start of each String (i.e. not Mob hashcodes) key against
        // partialName
     ...
     * 
     * @param player
     *            Player requesting the names - they don't need to be told that
     *            they're in the room
     * @return String containing a comma-separated list of all monsters and
     *         players in this room except the name of the player who's
     *         requesting the list
     */
    public String getMobAndPlayerNames(Player player) {
        ...
        // Scratch that: we need to be able to display "Mob" or "Dead mob", so
        // we'll
        // need mobs by object, not by name
        for (Object o : mobs.values()) {

            /*
             * now, here I could just tack on however many ", " + Mob names as I
             * needed and then chop off the first ", " with
             * StringBuffer.delete(0,2) Or, I can keep a boolean of whether or
             * not we need the ", " and set it to true after the first element.
             * I don't know which is faster. I'm assuming the branch is faster
             * than the StringBuffer alteration
             */
            if (bMultipleNames) {
            ...
            // carefull with the parenthesis on this one, the plus sign can mess
            // things up
            // strbMobNames.append(((Mob)i.next()).getName() +
            // (i.hasNext()?", ":""));
            // Scratch that: we'll just set a flag after we've added the first
            // name to say that we need
    ...
    public String getItemNames(boolean isDebug) {
        ...
        for (Item item : this.items.values()) {
            ...
            items.append(item.getName()).append(isDebug ? " [" + item.getId() + "]" : "");
            bMultipleItems = true;
        }
    ...
    /**
     * Stores the players in a hashmap so that they can be returned quickly by
     * name
     * 
     * @param pc
     *            PlayerChannel to add to the room
     * @see #add(Mob)
     * @see #add(Item)
     */
    public void add(PlayerChannel pc) {
        ...
        /*
         * we need to add the PlayerChannel based on the players hashcodeAND*
         * name. Why, you ask? Because, the whole point of having a hashtable is
         * so that we can pull out objects based on their name!
         * 
    ...
    /**
     * Add a <code>Mob</code> to this <code>Room</code> and index it by its
     * name, acronym and hashcode. (Why?)
     * 
     * @param m
     *            The <code>Mob</code> to add to this <code>Room</code>
     * @see #add(Item)
    ...
    public void add(Mob m) {
        ...
        try {
            ...
            System.out.println("Room (" + x + "," + y + "," + z + "): add(Mob m): Failed. "
                    + e.toString());
        }
    ...
    /**
     * Add an <code>Item</code> to this <code>Room</code> and index it by its
     * name and hashcode. (Why by hashcode?)
     * 
     * @param i
     *            The <code>Item</code> to add to this <code>Room</code>
     * @see #add(Mob)
     * @see #add(PlayerChannel)
     */
     ...
     * <p/>
     * The full name should be retrieved from the room before calling
     * getPlayerChannel(String name)
     * 
     * @param name
     *            The full name to match against all players in the
     *            <code>Room</code>
    ...
    public PlayerChannel getPlayerChannel(String name) {
        ...
        try {
            ...
            System.out.println("Room (" + x + "," + y + "," + z
                    + "): getPlayer(String name): Failed.");
            return null;
        }
    }
     ...
     * 
     * @param name
     *            The full name to match against all players in the room
    ...
    /**
     * Return any <code>Mob</code> in this <code>Room</code> with a name that
     ...
     * 
     * @param name
     *            The full name to match against all <code>Mob</code>s in the
     *            <code>Room</code>
    ...
    /**
     * Return any <code>Item</code> in this <code>Room</code> with a name that
     * matches <code>name</code>
     * 
     * @param name
     *            The name to match against all <code>Item</code>s in the
     *            <code>Room</code>
     * @return The first matching <code>Mob</code> if any, else null
     */
    ...
    /**
     * Removes a <code>PlayerChannel</code> (A <code>Player</code> and their
     * <code>SocketChannel</code>) from this <code>Room</code>
     * 
     * @param pc
     *            The <code>PlayerChannel</code> that contains the player to
     *            remove
     * @see #remove(Mob)
     * @see #remove(Item)
     */
    public void remove(PlayerChannel pc) {
        ...
        try {
            // hashtable elements must be removed with the key objects, not with
            // things that
            // hash out to the same value as the key object (I'm pretty sure
            // anyway)
     ...
     * 
     * @param m
     *            The <code>Mob</code> that is to be removed
     ...
     * 
     * @param i
     *            The <code>Item</code> that is to be removed
     ...
     * @throws Exception
     *             Throws any exception caused by ChannelWriter.sendMessage
     */
    // public void sendMessage(String strMessage, SocketChannel socketchannel)
    // throws Exception{
    ...
    public void sendMessageToAll(String strMessage) throws Exception {
        ...
        for (Enumeration e = playerChannels.elements(); e.hasMoreElements();) {
            ...
            /*
             * sendMessage(strMessage + playerChannel.getPlayer().getPrompt(),
    ...
    /**
     * Send a message to all <code>Players</code> in a <code>Room</code> except
     * one. This is to facilitate sending a message to all <code>Players</code>
     * but the one who is the initiator of the message. E.g. A
     * <code>Player</code> who chats, attacks, etc..
     * 
     * @param strMessage
     *            The message to send to all other players
     * @param playerChannelExcluded
     *            The playerChannel of the player NOT to send the message to
     * @throws Exception
     *             Throws any exception caused by ChannelWriter.sendMessage
     */
    public void sendMessageToAllButOne(String strMessage, PlayerChannel playerChannelExcluded)
            throws Exception {
        ...
        for (Enumeration e = playerChannels.elements(); e.hasMoreElements();) {
            ...
            // if the current PlayerChannel isn't the PlayerChannel we're
            // excluding then
