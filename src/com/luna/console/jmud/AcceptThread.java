...
/**
 * AcceptThread waits for incoming connections. When a connection is made, a
 * socket channel is created, added to a list of accepted connections and
 * registered with a selector that will wait for input on that channel.
 * 
 * @author Chris Maguire
 * @version 0.1
 *          <p/>
 *          ToDo No one else needs access to the accept (new connection)
 *          selector so we don't need to have it as a parameter: we can just
...
class AcceptThread extends Thread {
    ...
    private final ConnectionList acceptedConnections;

    /**
     * Open up a server socket channel, attach the server socket channel to a
     * port, attach a selector to the server socket channel that listens for
     * connections.
     * 
     * @param connectSelector
     *            A Selector that will watch for connections.
     * @param list
     *            A list of connected socket channels that other threads will be
     *            checking
     * @param port
    ...
    public AcceptThread(Selector connectSelector, ConnectionList list, int port) throws Exception {
        ...
        // open a server socket channel that will be able to create socket
        // channels for
    ...
    /**
     * Create new socket channels for incoming connections and add the socket
     * channels to the list of channels that other threads will be watching (the
     * push function also registers the new socket channel with the selector
     * associated with accepted connections).
     */
    protected void acceptPendingConnections() throws Exception {
        ...
        for (Iterator i = readyKeys.iterator(); i.hasNext();) {
            ...
            // create a socket channel and use the new ServerSocketChannel to
            // accept
            ...
            // Put the channel on the list of accepted channels and notify the
            // selector that
            // listens for input from accepted channels for login information

            // NOTE: Here's where we could limit the number of channels per
            // thread
            // although I don't know if we'd need to or want to unless we got to
            // 63
