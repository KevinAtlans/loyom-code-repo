...
public class Player implements Serializable, Target {
    ...
    /*
     * Store references to all "settable" methods both by aliases and by names.
     * Getting methods by alias allows us to pick a method and getting methods
     * (from a set) by name allows us to get a list of all the methods and
     * display their results with a human readable name.
     */
    static {
        Flag flag;
     ...
     * Creates new Player
     * <p/>
     * Also creates a StringBuffer for holding an unfinnished command in
     * progress
     */
    public Player() {
     ...
     * 
     * @param name
     *            The name (not login) to assign to this player. This is what
     *            other players in the game will see and refer to this player
     *            as.
     */
     ...
     *            as.
     * @param login
     *            The login name that the user will used to login to the game
     *            and control this player
     * @param password
     *            The password that will be used to ensure only the allowed
     *            users will be able to use this player (it isn't really
     *            security, but oh well)
     * @param currHitPoints
     *            The current amount of hit points this player should have, if
     *            more than Max number of hit points than the max number of hit
     *            points will be assigned to the player's current hit points
     *            instead.
     * @param maxHitPoints
     *            The maximum number of hit points this player should have. If
     *            the max hit points is less than one then max hit points will
     *            be set to one.
     * @param strength
     *            The quantifiable strength of the player
     * @param constitution
     *            The quantifiable constitution (overall healthfullness) of the
     *            player.
     * @param dexterity
     *            The quantifiable dexterity (overal agility, reflexes and
     *            speed) of the player.
     * @param inteligence
     *            The quantifiable intelligence of the player.
     * @param iID
     *            database ID of the player
     * @param desc
     *            description of the player
     */
    public Player(int iID, String name, String desc, String login, String password,
            int currHitPoints, int maxHitPoints, int strength, int constitution, int dexterity,
            int inteligence) {
    ...
    /**
     * Determines whether an instance of this class is equal to another instance
     * of this class.
     * 
     * @param o
     *            The object to compare this instance to (might not be of the
     *            same class)
     * @return true if the login of this instance matches the login of instance
     *         o or false if the class of o does not match the class of this
     *         instance or the login of class o does not match the login of this
     *         instance.
     */
    @Override
    ...
    /**
     * Get the hash code for this Player instance which is simply the hashcode
     * of the Player's login.
     * 
     ...
     * 
     * @param login
     *            The String containing the new login for this Player instance.
     */
     ...
     * 
     * @param login
     *            A login object (username and password) to match against this
     *            player
     * @return return true if the submitted login and password match this
     *         player's login and password, else false.
     */
    public boolean checkLogin(Login login) {

        // getLogin and getPassword return string buffers
        return login.getLogin().toString().equals(this.login)
                && login.getPassword().toString().equals(password);
    }
     ...
     * <p/>
     * Caution: this does not cancel any reference to the player that the room
     * may have.
     * 
     * @param rm
     *            The room to connect the player to, or put the player "in"
     */
     ...
     * <p/>
     * Note: The room or rooms that have a reference to this player may not be
     * the same as the room the player has a reference to. (Although they should
     * be, or something is wrong)
     * 
    ...
    /**
     * Return the player's Armour Class, the quantifiable amount of protection
     * that the player has from wearing armour.
     * 
    ...
    /**
     * Deduct hit points from the player and determine if the player has been
     * killed.
     * 
     * @param iDmg
     *            The amount of hit points to deduct from the players current
     *            hit points.
    ...
    /**
     * Add hit points to the player and determine how many hit points the player
     * now has.
     * <p/>
     * Trying to add more hit points to the players current hit points greater
     * than the difference between the player's max hit points and current hit
     * points will NOT cause the player to have more current than max hit
     * points. In other words, extra hit points are, in effect, wasted.
     * 
     * @param iHitPoints
     *            The number of hit points to add to the player's current hit
     *            points.
     ...
     * 
     * @param iHitPoints
     *            The number of hit points to raise the player's maximum hit
     *            points by
     ...
     * 
     * @param iHitPoints
     *            The number of hit points to lower the player's maximum hit
     *            points by
     * @return An int specifying what the resulting maximum number of hit points
     *         this player can have is
     */
    ...
    /**
     * Determine if the player is currently poisoned <br>
     * This must be rewritten to reflect the new poison resistance array
     * 
     * @return true if the player is poisoned, false if the player is not
     *         poisoned
     */
    public boolean getPoisoned() {

        /*
         * if(iPoisonStrength > 0){ return true; }else{ return false; }
         */
        return false;
    }
     ...
     * Poison the player
     * 
     * @param iPoisonStrength
     *            The strength of the poison to poison the player with
     * @return An int representing the total combined strength of all poisons
     *         this player has been poisoned with
     */
     ...
     * <p/>
     * If the healing strength of the cure is greater than the strength of the
     * total combined strength of the poison that the player has been poisoned
     * with the player does NOT maintain the "leftover" strength of the cure.
     * 
     * @param iHealingStrength
     *            The strength of the cure to counteract the poison that the
     *            player has been poisoned with.
     * @return An int representing the total combined strength of all poisons
     *         this player is still poisoned with
     */
    public int curePoison(int iHealingStrength) {

        /*
         * if(iPoisonStrength > iHealingStrength){ iPoisonStrength -=
         * iHealingStrength; }else{ iPoisonStrength = 0; }
         */
        return 1;
    }
    ...
    public boolean grab(Item item) {
        ...
        // if we can find a slot that is a grabber and isn't full then we're
        // good to go
