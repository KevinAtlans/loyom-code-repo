package com.luna.crawler.tencent.weibo.example;
...
import com.luna.crawler.tencent.weibo.api.FriendsAPI;
import com.luna.crawler.tencent.weibo.api.InfoAPI;
import com.luna.crawler.tencent.weibo.api.PrivateAPI;
import com.luna.crawler.tencent.weibo.api.SearchAPI;
import com.luna.crawler.tencent.weibo.api.StatusesAPI;
import com.luna.crawler.tencent.weibo.api.TAPI;
import com.luna.crawler.tencent.weibo.api.UserAPI;
import com.luna.crawler.tencent.weibo.oauthv1.OAuthV1;
import com.luna.crawler.tencent.weibo.oauthv1.OAuthV1Client;
...
public class TestOAuthV1GrantAndAPI {
    ...
    private void testList() throws Exception {
        ...
        response = tAPI.addPic(oAuth, format, "发表一条带本地图片的微博" + content, clientip, jing, wei,
                picpath, syncflag);
        // 取出返回的 id
        responseJsonObject = JSONObject.fromObject(response);
        dataJsonObject = (JSONObject) responseJsonObject.get("data");
        ...
        // 更多微博相关API测试
        tAPI.addPic(oAuth, format, "发表一条带网络图片的微博" + content, clientip, jing, wei,
        ...
        tAPI.addVideo(oAuth, format, "发表视频微博" + content, clientip, jing, wei,
        ...
        searchAPI.t(oAuth, format, keyword, pagesize, page, contenttype, sorttype, msgtype,
                searchtype, starttime, endtime, province, city, longitue, latitude, radius);
        ...
        statusesAPI.broadcastTimeline(oAuth, format, pageflag, pagetime, reqnum, lastid, type,
                contenttype);
        statusesAPI.homeTimeline(oAuth, format, pageflag, pagetime, reqnum, type, contenttype);
        statusesAPI.mentionsTimeline(oAuth, format, pageflag, pagetime, reqnum, lastid, type,
                contenttype);
        statusesAPI.usersTimeline(oAuth, format, pageflag, pagetime, reqnum, lastid, names,
                fopenids, type, contenttype);
        statusesAPI.userTimelineIds(oAuth, format, pageflag, pagetime, reqnum, lastid, name,
                fopenid, type, contenttype);
        statusesAPI.userTimeline(oAuth, format, pageflag, pagetime, reqnum, lastid, name, fopenid,
                type, contenttype);
    ...
    private static OAuthV1 verify(OAuthV1 oAuth) throws Exception {
        ...
        System.out
                .println("Input the authorization information (eg: oauth_token=OAUTH_TOKEN&vcode=VERIFY_CODE&openid=OPENID&openkey=OPENKEY)：");
