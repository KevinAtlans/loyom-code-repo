package com.luna.crawler.tencent.weibo.oauthv1;
...
import com.luna.crawler.tencent.weibo.exceptions.OAuthClientException;
import com.luna.crawler.tencent.weibo.utils.QHttpClient;
import com.luna.crawler.tencent.weibo.utils.QStrOperate;
...
public class OAuthV1Client {
    ...
    public static OAuthV1 requestToken(OAuthV1 oAuth) throws Exception {
        ...
        String queryString = getOauthParams(OAuthConstants.OAUTH_V1_GET_REQUEST_TOKEN_URL, "GET",
                oAuth.getOauthConsumerSecret(), "", oAuth.getParamsList());
        log.info("requestToken queryString = " + queryString);

        String responseData = Q_HTTP_CLIENT.simpleHttpGet(
                OAuthConstants.OAUTH_V1_GET_REQUEST_TOKEN_URL, queryString);
    ...
    public static boolean parseAuthorization(String responseData, OAuthV1 oAuth) throws Exception {
        ...
        for (int i = 0; i < tokenArray.length; i++) {
            if (tokenArray[i].startsWith("oauth_token=")) {
                oAuth.setOauthToken(tokenArray[i].substring(tokenArray[i].indexOf('=') + 1,
                        tokenArray[i].length()));
            }
            if (tokenArray[i].startsWith("vcode=")) {
                oAuth.setOauthVerifier(tokenArray[i].substring(tokenArray[i].indexOf('=') + 1,
                        tokenArray[i].length()));
            }
            if (tokenArray[i].startsWith("openid=")) {
                oAuth.setOpenid(tokenArray[i].substring(tokenArray[i].indexOf('=') + 1,
                        tokenArray[i].length()));
            }
            if (tokenArray[i].startsWith("openkey=")) {
                oAuth.setOpenkey(tokenArray[i].substring(tokenArray[i].indexOf('=') + 1,
                        tokenArray[i].length()));
            }
        }
    ...
    public static OAuthV1 accessToken(OAuthV1 oAuth) throws Exception {
        ...
        log.info("Getting Access Token ...... \n RequestToken = " + oAuth.getOauthToken()
                + "\nOauthVerifier = " + oAuth.getOauthVerifier());
    ...
    public static String getOauthParams(String url, String httpMethod, String consumerSecret,
            String tokenSecrect, List<NameValuePair> queryParamsList) {
    ...
    public static boolean parseToken(String response, OAuthV1 oAuth) throws Exception {
        ...
        for (int i = 0; i < tokenArray.length; i++) {
            if (tokenArray[i].startsWith("oauth_token=")) {
                tmpStr = tokenArray[i].substring(tokenArray[i].indexOf('=') + 1,
                        tokenArray[i].length());
    ...
    private static String generateSignature(URL url, String consumerSecret,
            String accessTokenSecret, String httpMethod, List<NameValuePair> queryParamsList) {
